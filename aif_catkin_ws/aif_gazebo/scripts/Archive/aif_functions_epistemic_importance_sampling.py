...
def get_likelihood(robot_id,observations,goals,agent_vars,prior,use_ep=False, consensus=False, alpha=0.9):
    ...
    posterior = np.array([0.8 * likelihood[i] + 0.2 * prior[i] for i in range(len(prior))])
...
def target_distribution(task_allocation, evidence):
    # Convert distances to probabilities using softmax
    probabilities = softmax(evidence) # Apply softmax along the goal dimension
    joint_prob = np.prod([probabilities[i, task] for i, task in enumerate(task_allocation)])
    return joint_prob

# @jit(nopython=True, parallel=True)
# @jit(nopython=True, parallel=True)
# @jit
def calculate_joint_probs(probabilities, reward_configs, num_agents, calc_type="new"):
    if calc_type == "new":
        ...
        likelihood_2 = np.array(joint_probabilities[tuple(reward_configs.T)])

    return likelihood

# @jit(nopython=True, parallel=False)
# def calculate_joint_probs(probabilities, reward_configs, num_agents):
#     num_configs = reward_configs.shape[0]
#     likelihood = np.ones(num_configs, dtype=np.float64)
    
#     for idx in prange(num_configs):
#         l_tmp = 1.0
#         for i in range(num_agents):
#             l_tmp *= probabilities[i, reward_configs[idx, i]]
#         likelihood[idx] = l_tmp 
...
def predict_agent_position(agent_position, velocity, heading, dt):
    ...
    heading = wrapToPi(agent_prediction[2] + heading)
...
def make_decision(agent_vars, goals, use_ep=True):
    ...
    return best_action, observations, best_value, observed_positions
...
def run_simulation(args, max_iterations=100):
    ...
    # Initialize the prior for each agent
    for idx in range(num_agents):
        agent_types_tmp = [agent_vars[idx]['agent_type'] for _ in range(num_agents)]
        agent_vars[idx]['goals'] = choose_subset_goals(goals_not_completed, agent_vars[idx]['agent_positions'][:,:2], agent_types_tmp, agent_vars[idx])
        agent_vars[idx]['reward_configs'] = identify_reward_configs(current_positions[:,:2], agent_types_tmp, agent_vars[idx]) 
    ...
    for iteration in range(max_iterations):
        ...
        for idx in range(num_agents):
            if not agent_vars[idx]['greedy']:
                agent_types = [agent_vars[idx]['agent_type'] for _ in range(num_agents)]
            ...
            decision, observation, free_energy_score, observed_positions = make_decision(agent_vars[idx], goals_not_completed, use_ep) # Make decision for agent
        ...
        for agent_id, (velocity, heading) in enumerate(decisions):
            true_positions[agent_id] = predict_agent_position(true_positions[agent_id], velocity, heading, dt)
        ...
        # Check if agents have converged to the same goal
        new_prior, goals_completed = delete_goals(goals, true_positions, goals_completed, args['max_distance_measure']/args['env_size'])
        if new_prior and not args['greedy']:
            for agent_id in range(num_agents):
                agent_vars[agent_id]['reward_configs'] = identify_reward_configs(observed_positions[:,:2], agent_types_tmp, agent_vars[idx]) 
                agent_vars[agent_id]['prior'] = set_initial_prior(agent_vars[agent_id])
        current_positions = np.copy(true_positions)
        convergence_check, selected_goal = check_convergence(current_positions, agent_vars[agent_id]['goals'], args['convergence_type'], goals_completed, args['max_distance_measure']/args['env_size'])
        ...
        if convergence_check:
            converged_count += 1
            if converged_count >= 1 or (args['greedy']):
                avg_execution_time = execution_time / (iteration + 1)
                print(f"\nAgents have converged to the same goal after {iteration + 1} iterations. EP: {args['use_ep']}, Greedy: {args['greedy']}. Avg Execution Time: {avg_execution_time}s")
...
def sample_reward_configs(positions, goals, obs_types, agent_params, num_samples):
    num_agents = positions.shape[0]
    num_goals = num_agents

    scores = custom_cdist(agent_params['agent_id'], positions, goals, obs_types, agent_params['max_distance_measure'])
    prob_matrix = compute_consensus(softmax(scores),obs_types).flatten()

    sampled_configs = []
    for _ in range(num_samples):
        config = []
        for i in range(num_agents):
            goal = np.random.choice(num_goals, p=prob_matrix)
            config.append(goal)
...
def identify_reward_configs(positions,obs_types, agent_params,N=200):
...
def greedy_decision(positions, goals, obs_types, agent_params, num_goals=1):
    ...
    goal_idx = np.argsort(agent_evidence,axis=0)[-num_goals:]
    goal = goals[goal_idx,:]
    return goal
...
def expansion(node, agent_params, goals):
    ...
    for velocity in velocity_options:
        for heading in heading_options:
            predicted_position = predict_agent_position(current_positions[agent_id], velocity, heading, agent_params['dt'])
...
def simulation(node, goals, agent_params, horizon):
    ...
    for _ in range(horizon):
        ...
        current_positions[agent_id] = predict_agent_position(current_positions[agent_id], velocity, heading, agent_params['dt'])       
