...
def make_decision(agent_vars, use_ep=True):
    ...
    if agent_vars['use_mcts'] and not use_threading:
        ...
        best_action, best_value = move_towards_goal(agent_id, observed_positions[:,:2], goal, agent_vars)
    else:
...
def move_towards_goal(agent_id, positions, goal, agent_params):
    ...
    if len(goal) == 0:
        return (0, 0), 0
    best_velocity = np.min((np.linalg.norm(goal - positions[agent_id]), agent_params['velocity_options'][-1]))
    best_heading = np.arctan2(goal[1] - positions[agent_id][1], goal[0] - positions[agent_id][0])
    best_heading = np.max((np.min((best_heading, agent_params['heading_options'][-1])), agent_params['heading_options'][0]))
    value = 0
