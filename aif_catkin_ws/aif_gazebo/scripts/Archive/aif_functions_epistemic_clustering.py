...
def get_likelihood(robot_id,observations,goals,agent_vars,prior,use_ep=False, consensus=False, alpha=0.9):
    ...
    posterior = np.array([0.9 * likelihood[i] + 0.1 * prior[i] for i in range(len(prior))])
...
def calculate_joint_goal_probs(agent_id, agent_poses, goals, predict_types, reward_configs, max_distance=30.0):
    ...
    # # Find the configuration where probabilities are the highest
    # allocate = linear_sum_assignment(-probabilities) # Hungarian algorithm to find the best allocation
    # reward_configs_opt = allocate[1] # Allocate the reward configurations based on the best allocation
    ...
    #This would be if hungarian works eventually
    # likelihood = np.array([np.sum(reward_configs_opt[i] * probabilities) for i in range(len(reward_configs))])
    ...
    for iteration in range(max_iterations):
        ...
        for idx in range(num_agents):
            if not agent_vars[idx]['greedy']:
                ...
                agent_vars[idx]['reward_configs'] = identify_reward_configs(len(agent_vars[idx]['goals']) )
            else: 
        ...
        if new_prior and use_ep:
            for agent_id in range(num_agents):
                agent_vars[agent_id]['prior'] = np.tile(1/len(args['reward_configs']), (len(args['agent_positions']), len(args['reward_configs']))) 
        elif new_prior and not use_ep:
            for agent_id in range(num_agents):
                agent_vars[agent_id]['prior'] = np.ones(len(args['reward_configs']), dtype=float) / len(args['reward_configs'])
        ...
        if convergence_check:
            ...
            if converged_count >= 1 or (args['greedy']):
                print(f"\nAgents have converged to the same goal after {iteration + 1} iterations.")
