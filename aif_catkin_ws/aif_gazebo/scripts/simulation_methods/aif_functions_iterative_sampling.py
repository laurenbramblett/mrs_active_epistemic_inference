...
def get_likelihood(robot_id,observations,goals,agent_vars,prior,use_ep=False, consensus=False, alpha=0.9):
    ...
    posterior = np.array([0.1 * likelihood[i] + 0.9 * prior[i] for i in range(len(prior))])
...
def custom_cdist(agent_id, x, goals, types, eta=30.0):
    ...
    alignment = 1 - (np.cos(relative_angles) + 1) / 2
    alignment[agent_id] = 1/len(goals)
    # targeting_counts = np.sum(alignment > 1./len(goals), axis=0)
    
    # Compute uniqueness penalty
    # uniqueness_penalty = np.exp(targeting_counts)

    # Penalize alignment and renormalize
    alignment = alignment / 1#uniqueness_penalty
...
@jit
def calculate_joint_probs(probabilities, reward_configs, num_agents):
    likelihood = np.ones(len(reward_configs), dtype=np.float64)
    for idx in range(len(reward_configs)):
        l_tmp = 1.0
        for i in range(num_agents):
            l_tmp *= probabilities[i][reward_configs[idx][i]]
        likelihood[idx] = l_tmp
    return likelihood

def calculate_joint_goal_probs(agent_id, agent_poses, goals, predict_types, reward_configs, max_distance=30.0, N=200):
    """Calculate the joint goal probabilities for any number of agents and goals, applying a reward to specified configurations."""
    ...
    likelihood = calculate_joint_probs(probabilities, reward_configs, num_agents)

    # Normalize the joint probabilities
    likelihood = softmax(np.array(likelihood))
...
def predict_agent_position(agent_position, velocity, heading):
    ...
    agent_prediction[2] = wrapToPi(agent_prediction[2] + heading)
...
def make_decision(agent_vars, goals, use_ep=True):
    ...
    if use_ep and not agent_vars['greedy']:
        consensus_prior = softmax(compute_consensus(prior, agent_vars['agent_types']))
    if len(goals) < agent_vars['num_agents']:
        agent_vars['greedy'] = True
...
def parse_args_by_agent(args):
    ...
    agent_vars = []  # Initialize an empty list to store variables for each agent
    
    # Loop through each agent based on the number of agent positions provided in args
    for agent_id in range(len(args['agent_positions'])):
        
        # Create a dictionary to store variables specific to the current agent
        agent_dict = {
            'agent_id': agent_id,  # Unique identifier for the agent
            'num_agents': len(args['agent_positions']),  # Total number of agents in the simulation
            'home_base': args['home_base'],  # The home base position for the agents
            'agent_type': args['agent_types'][agent_id],  # The type of the agent (e.g., 's' for stationary, etc.)
            'agent_positions': args['agent_positions'],  # Current positions of all agents
            'goals': np.array(args['goals']),  # The goals that the agents need to reach, converted to a NumPy array
            'velocity_options': args['velocity_options'],  # Available velocity options for the agent
            'heading_options': args['heading_options'],  # Available heading (direction) options for the agent
            'num_actions': args['num_actions'],  # The number of possible actions the agent can take
            'max_distance_measure': args['max_distance_measure'],  # The maximum distance for goal completion
            'observation_error_std': args['observation_error_std'],  # The standard deviation of observation errors
            'prior': args['prior'],  # The prior belief of the agent about the goals or environment
            'use_ep': args['use_ep'],  # Flag indicating whether epistemic planning (EP) is used
            'use_mcts': args['use_mcts'],  # Flag indicating whether Monte Carlo Tree Search (MCTS) is used
            'agent_types': args['agent_types'],  # List of all agent types in the simulation
            # 'reward_configs': args['reward_configs'],  # (Optional) Reward configurations, if needed
            'mcts_iterations': args['mcts_iterations'],  # Number of iterations for MCTS if used
            'horizon': args['horizon'],  # Planning horizon for the agent's decision-making process
            'use_threading': args['use_threading'],  # Flag indicating whether threading is used
            'use_rhc': args['use_rhc'],  # Flag indicating whether receding horizon control (RHC) is used
            'greedy': args['greedy'],  # Flag indicating whether the agent uses a greedy strategy
            'dt': args['dt'],  # Time step for the simulation
        }
        
        # Optionally, initialize the agent's prior belief (commented out here)
        # agent_dict['prior'] = set_initial_prior(agent_dict)
        
        agent_vars.append(agent_dict)  # Add the current agent's dictionary to the list of agent variables
    
    return agent_vars  # Return the list of agent-specific variables


def set_initial_prior(args):
    if args['use_ep']:
        prior = np.tile(1/len(args['reward_configs']), (len(args['agent_positions']), len(args['reward_configs'])))      
    else:
        prior = np.ones(len(args['reward_configs']), dtype=float) / len(args['reward_configs'])
    return prior

def check_convergence(positions, goals, convergence_type, goals_completed, max_distance=1.0):
    """Check if agents have converged to the same goal."""
    if len(goals) == 0:
        return True, np.zeros(len(positions))
...
def run_simulation(args, max_iterations=100):
    ...
    return_args = {}  # Dictionary to store results for plotting and analysis
    num_agents = len(args['agent_positions'])  # Number of agents in the simulation
    
    # Initialize agent-specific variables
    agent_vars = parse_args_by_agent(args)
    
    # Initialize current and true positions of the agents
    current_positions = np.copy(args['agent_positions'])
    true_positions = np.copy(current_positions)
    
    # List of goals that agents need to achieve
    goals = args['goals']
    
    # Initialize lists to store the positions, headings, and goal completion status over time
    ...
    # List to track free energy scores for each agent at each iteration
    free_energy_scores = []
...
<<<<<<< HEAD
    ...
    converged_count = 0  # Counter to track how many agents have converged to the same goal
    execution_time = 0  # Variable to track the total execution time of the simulation
    goals_completed = np.copy(return_args['completed_goals'][-1])  # Initialize goals completed array

    for iteration in range(max_iterations):
        # At each iteration, make decisions for all agents
        start_time = time.time()  # Record the start time of the iteration
        
        decisions = []  # List to store decisions (velocity, heading) for each agent
        observations = []  # List to store observations made by each agent
        
        goals_not_completed = goals[~goals_completed]  # List of goals that are still incomplete
        
        for idx in range(num_agents):
            if not agent_vars[idx]['greedy']:
                # Update goals and reward configurations for non-greedy agents
                agent_types = [agent_vars[idx]['agent_types'][i] for i in range(num_agents)]
                agent_types[idx] = 's'
                agent_vars[idx]['goals'] = choose_subset_goals(goals_not_completed, agent_vars[idx]['agent_positions'][:,:2], agent_types, agent_vars[idx])
                agent_vars[idx]['reward_configs'] = identify_reward_configs(agent_vars[idx]['agent_positions'], agent_vars[idx])
                if iteration == 0:
                    agent_vars[idx]['prior'] = set_initial_prior(agent_vars[idx])
            else:
                # For greedy agents, consider all incomplete goals
                agent_vars[idx]['goals'] = np.copy(goals_not_completed)
            
            # Make a decision based on the current state and update the free energy score
            decision, observation, free_energy_score = make_decision(agent_vars[idx], goals_not_completed, use_ep)
            decisions.append(decision)  # Store the decision made by the agent
            observations.append(observation)  # Store the observation made by the agent
            free_energy_scores.append((free_energy_score, idx, iteration))  # Store the free energy score for analysis
        
        # Update the positions of the agents based on their decisions
        for agent_id, (velocity, heading) in enumerate(decisions):
            new_heading = wrapToPi(true_positions[agent_id][2] + heading)  # Calculate the new heading
            dx = velocity * np.cos(new_heading) * dt  # Calculate the change in x position
...
=======
    ...
    for iteration in range(max_iterations):
        ...
        goals_not_completed = goals[~goals_completed]
        if not convergence_check:
            for idx in range(num_agents):
                if not agent_vars[idx]['greedy']:
                    ...
                    agent_vars[idx]['goals'] = np.copy(goals_not_completed)
                decision, observation, free_energy_score = make_decision(agent_vars[idx], goals_not_completed, use_ep) # Make decision for agent
            ...
            for agent_id in range(num_agents):
                ...
                if not agent_vars[agent_id]['greedy']:
                    agent_prior = agent_vars[agent_id]['prior'].copy()
                    agent_vars[agent_id]['prior'] = get_likelihood(agent_id,observations[agent_id], agent_vars[agent_id]['goals'], 
                                                                agent_vars[agent_id], agent_prior, use_ep)
                        
            # Check if agents have converged to the same goal
            new_prior, goals_completed = delete_goals(goals, true_positions, goals_completed, 0.5)#args['max_distance_measure']/args['env_size'])
            if new_prior and not agent_vars[0]['greedy']:
                for agent_id in range(num_agents):
                    agent_vars[agent_id]['prior'] = set_initial_prior(agent_vars[agent_id])

        current_positions = np.copy(true_positions)
        convergence_check, selected_goal = check_convergence(current_positions, agent_vars[agent_id]['goals'], args['convergence_type'], goals_completed, 0.1)#args['max_distance_measure']/args['env_size'])
        # Get execution time and print progress
        end_time = time.time()
        execution_time += end_time - start_time
        print(f"\rIteration {iteration+1}: Total execution time: {execution_time:.2f}s", end="")
        ...
        if convergence_check:
            ...
            if converged_count >= 10:
                print(f"\nAgents have converged after {iteration + 1} iterations with average execution time of {execution_time / (iteration + 1)}s.")
    ...
    return {'positions': current_positions, 'converged': False, 'iteration': iteration, 
            'plot_args': return_args, 'priors': [agent_vars[i]['prior'] for i in range(num_agents)], 'energy_scores': free_energy_scores}
>>>>>>> c61e494b9be0e14693bf418f5bca1d8a6970395a

#------------------------------------------------------------------------------------------
# ----------------- Functions for Epistemic Allocation ------------------------------------
#------------------------------------------------------------------------------------------

def choose_subset_goals(goals, agent_positions, agent_types, params):
    """Choose a subset of goals for each agent to consider."""
    agent_id = params['agent_id']
    goals_to_output = np.min((goals.shape[0], params['num_agents']))
    if len(goals) < params['num_agents']:
        goal = greedy_decision(agent_positions, params['goals'], agent_types, params)
        if len(goal) == 0:
            goal = params['home_base']
        tmp_goals = np.repeat(params['home_base'], params['num_agents'] - goals.shape[0], axis = 0).reshape(params['num_agents'] - goals.shape[0], 2)
        goals = np.concatenate((goals, tmp_goals), axis=0)
    else:
        goals = greedy_decision(agent_positions, goals, agent_types, params, goals_to_output)

    return goals

def identify_reward_configs(positions,agent_params,N=200):
    ...
    num_goals = goals.shape[0]
    if num_agents < 5:
        tuple_elements = [i for i in range(num_goals)]
        configurations = list(itertools.permutations(tuple_elements))
    else:
        # Choose informed sample
        # configurations = sample_reward_configs(positions, goals, obs_types, agent_params, N)
        configurations = [np.random.permutation(num_agents) for _ in range(N)]
    reward_configs = np.array(configurations)
...
def choice_heuristic(current_positions, observations, prior, agent_params, use_ep=False, consensus=False):
    ...
    # free_energies = np.array(entropies) + np.array(kl_divergences) + 10e-4*np.array(min_distance) #Min distance breaks ties
...
class PlotSim_fast:
    ...
    def __init__(self, num_agents, goals, robot_types, env_size=15, padding = 5, scale = 0.5):
        ...
        self.scale = scale
        self.cmap = Set1_9.mpl_colors
        self.agent_paths = [self.ax.plot([], [], '-', markersize=3, linewidth=3, alpha=1, color=self.cmap[i])[0] for i in range(num_agents)]
        self.goal_plots = [self.ax.plot(goal[0], goal[1], '*', markersize=12, color='k',markerfacecolor='purple')[0] for goal in goals]  # Plot goals
        self.all_rosbots = []  # List to store all robots
        self.robot_types = robot_types
        self.initial_task_positions = goals  # Store initial task positions
        self.completed_tasks = [False] * len(goals)  # Initialize all tasks as not completed
    ...
    def update(self, frame, args):
        ...
        for task_id, completed in enumerate(task_status):
            if completed:
                self.goal_plots[task_id].set_markerfacecolor('gray')
