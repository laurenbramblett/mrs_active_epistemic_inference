...
def softmax(x):
    ...
    return e_x / (np.sum(e_x, axis=1, keepdims=True) + 1e-32)

def log_stable(x):
    """Compute log values for each sets of scores in x."""
    return np.log(x + np.exp(-32))
...
def calculate_goal_reward(agent_id, q, x, goals, reward_configs, max_distance=30.0):
    ...
    chosen_goal = np.argmax(aggregate_post)
    chosen_config = reward_configs[chosen_goal]
    goal_config = chosen_config[agent_id]
    closest_goal = np.linalg.norm(goals[goal_config] - x[:2])/max_distance
    return closest_goal
...
def calculate_kl_divergence(q):
    ...
    return np.sum(p * log_stable(p / (q + np.exp(-32)))) #Divide by 16 to normalize the KL divergence
...
def get_likelihood(robot_id,observations,goals,agent_vars,prior,use_ep=False, consensus=False, alpha=0.9):
    ...
    if not use_ep:
        ...
        for predict_id, predict_type in enumerate(agent_vars['agent_types']):
            tmp_obs_type = robot_types.copy()
            tmp_obs_type = ['none' if (predict_type == 'A' and robot_types[robot_id] == 'B') else robot_types[predict_id] for _ in range(len(robot_types))]
            tmp_obs_type[predict_id] = 's' if predict_id == robot_id or robot_types[robot_id] == 'A' else tmp_obs_type[predict_id]
    ...
    if consensus:
        robot_types_tmp = robot_types.copy()
        robot_types_tmp[robot_id] = 's'
        # prior = compute_consensus(prior, robot_types_tmp)
        likelihood = compute_consensus(likelihood, robot_types_tmp)

    posterior = np.array([(0.5 * likelihood[i]) + (0.5 * prior[i]) for i in range(len(prior))])

    return softmax(posterior)

def compute_consensus(likelihoods, agent_types):
    """Compute consensus likelihoods based on agent types."""
    consensus_likelihoods = np.zeros(likelihoods.shape[1], dtype=float)
    # weights = np.array([2.0 if agent_type == 's' else 0.5 for agent_type in agent_types])
    consensus_weights = np.clip([calculate_shannon_entropy(d) for d in likelihoods],0,20)

    for idx in range(likelihoods.shape[0]):
        consensus_likelihoods += np.array(likelihoods[idx,:] * consensus_weights[idx])
...
def custom_cdist(agent_id, x, goals, types, eta=30.0):
    ...
    evidence_to_goals = softmax(np.exp(-1/eta * distances_to_goals))
    ...
    alignment = np.array([softmax(- np.squeeze(a)/ np.pi) for a in relative_angles])
...
def make_decision(agent_vars, use_ep=True):
    ...
    if use_ep:
        consensus_prior = compute_consensus(prior, agent_vars['agent_types'])
...
def choice_heuristic(current_positions, observations, prior, agent_params, use_ep=False, consensus=False):
    ...
    free_energies = np.array(entropies) + kl_divergences#1e-10*np.array(min_distance)
