...
def get_likelihood(robot_id,observations,goals,agent_vars,prior,use_ep=False, consensus=False, alpha=0.9):
    ...
    posterior = np.array([(0.3 * likelihood[i]) + (0.7 * prior[i]) for i in range(len(prior))])
...
def check_convergence(positions, goals, convergence_type, max_distance=1.0):
    ...
    # print("Positions and goals: ", positions, goals)

    # Calculate the distances from each position to each goal
    distances_to_goals = np.array([[np.linalg.norm(goal - pos[:2]) for goal in goals] for pos in positions])
    
    # Find the minimum distance for each position
    distances_to_selected_goal = np.min(distances_to_goals, axis=1)
    selected_goals = np.argmin(distances_to_goals, axis=1)

    if convergence_type == 'convergent':
        # print("Which goal: ", selected_goals)
        all_same_goal = np.all(selected_goals == selected_goals[0])
        check = (distances_to_selected_goal < max_distance).all() and all_same_goal
        return check, selected_goals

    elif convergence_type == 'exclusive':
        all_different_goals = len(selected_goals) == len(set(selected_goals))
        check = (distances_to_selected_goal < max_distance).all() and all_different_goals
        return check, selected_goals
...
def choice_heuristic(current_positions, observations, prior, agent_params, use_ep=False, consensus=False):
    ...
    free_energies = np.array(entropies) + np.array(kl_divergences) + proximity_cost#10e-4*np.array(min_distance) + proximity_cost #Min distance breaks ties
