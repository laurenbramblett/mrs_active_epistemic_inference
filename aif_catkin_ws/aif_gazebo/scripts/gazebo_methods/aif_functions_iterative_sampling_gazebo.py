...
def calculate_kl_divergence(q):
    ...
    return np.sum(p * log_stable(p / (q + np.exp(-16))))*10e1 #Divide by 16 to normalize the KL divergence

def calculate_shannon_entropy(p):
    """Calculate Shannon entropy of a probability distribution."""
    return -np.sum(p * log_stable(p))*10e2
...
def predict_agent_position(agent_position, velocity, heading, dt=0.1):
...
def run_simulation(args, max_iterations=100):
    ...
    return_args = {}
    num_agents = len(args['agent_positions'])
    # args['reward_configs'] = identify_reward_configs(num_agents)
    # args['prior'] = np.ones(len(args['reward_configs']), dtype=float) / len(args['reward_configs'])
...
def choose_subset_goals(goals, agent_positions, agent_types, params):
    ...
    if len(goals) < params['num_agents']:
        ...
        tmp_goals = tmp_goals + np.random.normal(0, 0.1, tmp_goals.shape) # Add small differentiation to the home base
    ...
    # print("Agent ", agent_id, " has chosen goals ", goals)
...
def identify_reward_configs(positions,agent_params,N=200):
    ...
    num_goals = goals.shape[0]
    if num_agents < 5:
        tuple_elements = [i for i in range(num_agents)]
...
def choice_heuristic(current_positions, observations, prior, agent_params, use_ep=False, consensus=False):
    ...
    proximity_cost = [calculate_proximity_penalty(agent_params['agent_id'], predicted_observations[i],agent_params['convergence_threshold']+0.2) for i in range(len(agent_positions))]
    free_energies = np.array(entropies) + np.array(kl_divergences) + 10e-8*np.array(min_distance) + proximity_cost #Min distance breaks ties
