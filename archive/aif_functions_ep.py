...
def simulate_observation(true_position, observation_error_std=2.0, sim_type='A', observed_agent= None):
    ...
    if sim_type == 'self': # Assume no noise for self-observation
...
def get_likelihood(robot_pose,observations,goals,max_distance_measure=1.0,system_dict=None, use_ep=True, predict_self=False):
...
def salience(robot_pose, observation, goals, max_distance_measure=1.0, system_dict=None, use_ep=True, predict_self = False):
...
def run_simulation(args, max_iterations=100):
    ...
    for iteration in range(max_iterations):
        ...
        if (np.array(distances_to_selected_goal)<1.0).all() and all(all_same_goal):
