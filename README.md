...
```
rosrun rviz rviz -d <rviz_path> # Example included in: ~/jackal_ws/src/aif_package/mrs_active_epistemic_inference/aif_catkin_ws/aif_gazebo/rviz/two_jackal_rviz.rviz
```
...
The yaml file in the configs folder (~/mrs_aif_ws/mrs_active_epistemic_inference/aif_catkin_ws/aif_gazebo/configs/) includes an example `aif_lab_params.yaml`. Update the pose topic names and namespaces as needed. You may also manipulate the transformed poses.
...
changes to code will occur in the `~/aif_catkin_ws/mrs_active_epistemic_inference/aif_gazebo/scripts/experiment_methods/aif_functions_isobeliefs_convergent_lab.py`
